...
class M4_db(M4_base):
    ...
    def __init__(self):
        ...
        self.analysis_table = 'ng_analysis'     # set to a table like analysis_gsd for debugging
    ...
    def load_gcwerks(self, gas, start_date, stop_date='end'):
        ...
        """
        m4_start_dt = pd.to_datetime(self.m4_start_date)
...
def main():
    ...
    if args.extract:
        m4.M4_GCwerks_Export().export_gc_data(start_date, molecules)
    
    for n, gas in enumerate(molecules):
        df = m4.load_gcwerks(gas, start_date)
        #print(df.loc[df['time'] > '2024-08-28 03:11:00'])
        m4.tmptbl_fill(df)             # create and fill in temp data table with GCwerks results

        #tmp = pd.DataFrame(m4.tmptbl_output())
        #print(tmp.loc[tmp.analysis_num >= 317010][['analysis_datetime', 'analysis_num', 'sample_type']])

        m4.tmptbl_update_flags_internal()  # need to call this before analysis rows are added.
        m4.tmptbl_update_analysis()    # insert and update any rows in hats.analysis with new data
        m4.tmptbl_update_raw_data()    # update the hats.raw_data table with area, ht, w, rt
        m4.tmptbl_update_ancillary_data()  # updates the hats.ancillary table with p, p0, pnet, and t1 values
