...
>1 page via pdfseparate, and moves originals into a 'converted/' subdir.

Triggering `checkin process` on the results is not this script's job -
checkin-process.path/.timer (see CLAUDE.md "Automated scanning") watches
the same incoming/ directory and picks up the split pages on its own.
"""
...
def process_once(work_dir: Path, converted_dir: Path, dry_run: bool) -> list[Path]:
    """One sweep: split every multi-page PDF found, return the new page paths."""
...
def main() -> int:
    ...
    split_outputs = process_once(work_dir, converted_dir, args.dry_run)
    if split_outputs:
        print(f"Split {len(split_outputs)} PDF page(s).")
    else:
